	...
	<div id="main">
		...
		<div id="head_of_table">
			<sec:authorize access="hasRole('ROLE_ADMIN')">
				<button id="new_book">Create new book</button>
			</sec:authorize>
			<h1>Books list</h1>
		</div>
		<div id="books_table">
				...
				<c:forEach items="${books}" var="book">
					<tr>
						...
						<td><c:forEach items="${book.authors}" var="author">
								<p>${author.name} ${author.secondName}</p><br>
							</c:forEach></td>
						<sec:authorize access="hasRole('ROLE_ADMIN')">
							<td>
								<a href="${contextPath}/modify_book/${book.id}.html">Modify</a>
	...
	<sec:authorize access="hasRole('ROLE_ADMIN')">
		<script type="text/javascript">
			$('#new_book').click(function() {
				window.location.href = '${contextPath}/create_book.html';
			})
		</script>
	</sec:authorize>
</body>
